 ...
 * S0001      VIKALP PATEL     27/02/14         SECURITY        ADDED PASSHAS IN JSON.
 * S0002      VIKALP PATEL     27/02/14         SECURITY        VALID USER CHECK [PASSWORD - ACCOUNT EXPIRED - DEVICE ALLOWED]
 * 1000F      VIKALP PATEL     05/03/14         RELEASE         ADDED DEPOSITED BANK LOV
 * P3A02      VIKALP PATEL     21/03/14         PHASE-III       ADDED IS SHARED PARAMETER IN JSON
 * P3A03      VIKALP PATEL     08/05/14         PHASE-III       ADDED IS SHARED PRIVATE PARAMETER IN JSON
 * P3A04      VIKALP PATEL     08/05/14         PHASE-III       ADDED CONTACT SHARED PRIVATE JSON
 * --------------------------------------------------------------------------------------------------------------------
 */
...
public class RequestBuilder {
	...
	public static JSONObject getLoginData(String username, String password,String device_id, String device_name, String myVersion)//EDITED X0001
	{
		...
		try
		{
			...
			stringBuffer.put("device_name", device_name);//SA S0002
			stringBuffer.put("os_version", myVersion);
			stringBuffer.put("device_id", device_id);//EA S0002
	...
	public static JSONObject getServicesData(String imei, JSONObject jsonObject, JSONObject tables)
	{
		...
		try
		{
			...
			stringBuffer.put("hash", SmartConsultant.getPreferences().getUserLoginDTO().getPassHash());//ADDED S0001
	...
	public static JSONArray getBankJSON(ArrayList<lovDTO> lov)
	{
		...
		try
		{
			for(int i = 0 ; i < lov.size(); i++)
			{
				...
				temp.put("bank_name", lov.get(i).getText());
...
//	SA 1000F
	public static JSONArray getDepositedBankJSON(ArrayList<lovDTO> lov)
	{
		JSONArray jsonArra = new JSONArray();
	//	JSONObject jsonObject = new JSONObject();
		try
...
//	EA 1000F
	...
	//EA 1000A
	//SA P3A04
		public static JSONObject getPatientContactDetails(PatientShareDetailsDTO dto)
		{
			try
			{
				JSONObject jsonObject = new JSONObject();
//				jsonObject.put("service_contact_id", dto.get_id());
				jsonObject.put("service_id", dto.getPatientId());
				jsonObject.put("contact_name", dto.getContactName());
				jsonObject.put("contact_number", dto.getContactNumber());
				return jsonObject;
			}
			catch (Exception e) {
				// TODO: handle exception
	...
	public static JSONObject getSxData(PatientDTO expenseDTOs)
	{
		...
		try
		{
			...
			jsonObject.put("referred_by", expenseDTOs.getRef());
			...
			jsonObject.put("is_shared", expenseDTOs.getIs_shared());//ADDED P3A02
			jsonObject.put("is_shared_private", expenseDTOs.getIs_shared_private());//ADDED P3A03
		...
		if(expenseDTOs != null && expenseDTOs.size() > 0)
		{
			for(int i = 0 ; i < expenseDTOs.size();i++)
			{
				...
				try
				{
					...
					jsonObject.put("is_shared", expenseDTOs.get(i).getIs_shared());//ADDED P3A02
					jsonObject.put("is_shared_private", expenseDTOs.get(i).getIs_shared_private());//ADDED P3A03
					jsonArray.put(jsonObject);
				}
				catch (Exception e) {
					// TODO: handle exception
