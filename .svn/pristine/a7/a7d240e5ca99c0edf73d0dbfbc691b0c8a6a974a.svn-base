...
* F0001       VIKALP PATEL       03/03/2014         FLOW            RE-FLOWED SYNC SCREEN
* P3B01       VIKALP PATEL       16/04/2014         BUG             SETTING ALARM	
* P3B02       VIKALP PATEL       21/04/2014         BUG             AGRESSIVE START OF SERVICE
* --------------------------------------------------------------------------------------------------------------------
*/
...
public class SocialLoginScreen extends Activity {
...
//				SmartConsultant.getPreferences().setUserLoginDTO(res);
		...
		protected void onPostExecute(UserLoginDTO result) {
			...
			if(result!=null && result.isStatus())
			{
				...
				setResult(RESULT_OK);
//				ServiceAlarm.startServiceAlarm("0"); //ADDED P3B01 ALARM
				ServiceAlarm serviceAlarm = new ServiceAlarm();//SA P3B02
				serviceAlarm.startServiceAlarm("0");//EA P3B02
				finish();
			}
			else
			{
...
//						CustomToast.showToastMessage(SmartConsultant.getApplication().getApplicationContext(), errorvalue);
